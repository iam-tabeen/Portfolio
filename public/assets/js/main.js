...
(function ($) {
  ...
  var tmPk = {
    ...
    methods: function (e) {
      tmPk.shapeMove();
      // tmPk.bgActiveBlur(); // removed — intro video disabled
      tmPk.serviceWidget();
      tmPk.swiperJs();
      tmPk.wowActive();
      ...
      // tmPk.rightDemo(); // removed — demo button disabled
      tmPk.onePageNav();
    ...
    tmpTiltAnimation: function () {
      ...
      if (tiltActive.length) {
        // Exclude portfolio cards — tilt on 30 cards causes scroll lag with Lenis
        $('.paralax-image:not(.latest-portfolio-area .paralax-image)').tilt({
          maxTilt:    5,
          speed:      1000,
    ...
    portfolioLivePreview: function () {
      var modal = document.getElementById('exampleModalCenter');
      if (!modal) return;
      var savedScroll = 0;
      modal.addEventListener('show.bs.modal', function (e) {
        // Save scroll position before modal opens
        savedScroll = (window.lenis && typeof window.lenis.scroll === 'number')
          ? window.lenis.scroll
          : (window.scrollY || 0);
        var trigger = e.relatedTarget;
        if (!trigger) return;
        // update modal image to match card image
        var imgSrc = trigger.getAttribute('data-img');
        var modalImg = modal.querySelector('.portfolio-popup-thumbnail .image img');
        if (modalImg && imgSrc) modalImg.setAttribute('src', imgSrc);
        // update title
        var titleEl = document.getElementById('portfolio-modal-title');
        if (titleEl) titleEl.textContent = trigger.getAttribute('data-title') || '';
        // update description
        var descEl = document.getElementById('portfolio-modal-desc');
        if (descEl) descEl.textContent = trigger.getAttribute('data-description') || '';
        // update live preview button
        var url = trigger.getAttribute('data-live-url');
        var btn = document.getElementById('portfolio-live-preview-btn');
        if (btn) btn.href = url && url !== '#' ? url : '#';
        // update tech pills
        var tagsRaw = trigger.getAttribute('data-tags') || '';
        var tagsContainer = document.getElementById('portfolio-modal-tags');
        if (tagsContainer) {
          tagsContainer.innerHTML = tagsRaw
            .split(',')
            .filter(function(t){ return t.trim(); })
            .map(function(t){ return '<span class="portfolio-modal-tag">' + t.trim() + '</span>'; })
            .join('');
        }
      });
      // Restore scroll position after modal closes (fixes Lenis scroll-to-top on close)
      modal.addEventListener('hidden.bs.modal', function () {
        var pos = savedScroll;
        if (window.lenis && typeof window.lenis.scrollTo === 'function') {
          window.lenis.scrollTo(pos, { immediate: true, duration: 0 });
        }
    ...
    odoMeter: function () {
      ...
      $(document).ready(function () {
        function isInViewport(element) {
          ...
          // Element is in viewport if any part of it is visible (partial visibility check)
          return rect.top < viewH && rect.bottom > 0;
        }
